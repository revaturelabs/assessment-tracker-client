...
function getAssessmentData(traineeId = 0) {
    //set the caller_complete to the function that is supposed to receive the response
    //naming convention: [this function name]_complete
    let response_func = getAssessmentData_complete;
    //endpoint: rest api endpoint
    let endpoint = "assessments";
    // if we want to get assessments for a specific person, we need to ensure that someone actually inputs
    // which week they wish to view
    if (traineeId > 0){
        endpoint = "assessments/" + traineeId;
    }
    //set the url by adding (base_url/java_base_url) + endpoint
    //options:
    //base_url(python)
